...
GamePlayManager = {
  ...
  preload: function () {
    ...
    game.load.image('booble2', 'assets/images/booble1.png');
  ...
  create: function () {
    ...
    this.mollusk = game.add.sprite(550,150,'mollusk');
    ...
    for (var i = 0; i < AMOUNT_DIAMONDS; i++) {
      ...
      var currentDiamond = this.getBounce(diamond);
      var rectHorse = this.getBounce(this.horse);

      while (this.isOverlappingOtherDiamond(i, currentDiamond) || this.isRectangleOverlapping(rectHorse, currentDiamond)) {
        diamond.x = game.rnd.integerInRange(50, 1050);
        diamond.y = game.rnd.integerInRange(50, 600);
        currentDiamond = this.getBounce(diamond);
      }
    }
  ...
  increaseScore: function(){
    ...
    if(this.amountDiamonds >= AMOUNT_DIAMONDS ){
      game.time.events.remove(this.timerGameover);
      this.endgame = true;
  ...
  onTap: function () {
    ...
    this.flagFirstMouseDowm = true;
  },
  getBounce: function (currentDiamond) {
  ...
  isOverlappingOtherDiamond: function (index, rect2) {
    for (var i = 0; i < index; i++) {
      var rect1 = this.getBounce(this.diamonds[i]);
  ...
  update: function () {
    ...
    if (this.flagFirstMouseDowm && !this.endgame) {
      ...
      this.horse.x += distX * 0.01;
      this.horse.y += distY * 0.01;

      for (var i = 0; i < AMOUNT_DIAMONDS; i++) {
        var rectHorse = this.getbounceHorse();
        var rectDiamonds = this.getBounce(this.diamonds[i]);

        if (this.diamonds[i].visible && this.isRectangleOverlapping(rectHorse, rectDiamonds)) {
          this.increaseScore();
          ...
          if (explosion != null) {
            ...
            explosion.tweenAlpha.onComplete.add(function (currentTarget, xcurrentTween) {
              currentTarget.kill();
            }, this);
          }
